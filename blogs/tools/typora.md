...
转发自：**https://www.52pojie.cn/forum.php?mod=redirect&goto=findpost&ptid=2084047&pid=54623222**
...
### 打开Tyroa，激活->输入序列表->离线激活->复制机器码->关闭程序

### 复制下面的脚本，创建`pj.js`文件

### 修改209行的路径为Typora安装路径 const Typora_Installation_Path = “D:\Typora”;
...
【建议关闭】是否开启调试？(Y/N): 
N
deviceId: YXD | yxd-l | Windows
fingerprint: e2hljTJhPt
version: win|1.12.4
已关闭所有 Typora.exe 进程
...
一、正在进行反反调试操作...
解包 asar
复制 app 到 app.bak（递归复制）【应对完整性校验】
移除 app.asar 文件
修改 Typora.exe 的 fuse 配置，允许加载未打包的 app 目录
反反调试操作完成！
二、正在注入破解代码到 launch.dist.js...
成功插入破解代码到 launch.dist.js
注入破解代码完成！
三、正在修改注册表以关闭联网验证...
(node:66488) [DEP0190] DeprecationWarning: Passing args to a child process with shell option true can lead to security vulnerabilities, as the arguments are not escaped, only concatenated.
...
后续操作建议：
        ...
        1.您的注册码为：+8BUAVX9B# 请复制并用于激活。
        2. 关闭【自动检查更新】功能，防止被覆盖。
